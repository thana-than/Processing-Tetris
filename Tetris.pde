...
color lineClearColor = color(125,125,125);
color ghostColor = color(125,125,125);
...
boolean resetButton() { return key == 'r'; }
boolean pauseButton() { return keyCheck('p', ENTER); }
boolean swapHoldButton() { return keyCheck('c', SHIFT); }
boolean hardDropButton() { return key == ' '; }
boolean softDropButton() { return keyCheck('s', DOWN); }
boolean moveLeftButton() { return keyCheck('a', LEFT); }
boolean moveRightButton() { return keyCheck('d', RIGHT); }
boolean rotateLeftButton() { return keyCheck('z', CONTROL); }
boolean rotateRightButton() { return keyCheck('x', UP); }
...
int drawCompressedBlocks(Block[] queue, Vector2 offset)
{
...
Vector2[] GetCompressedShape(Block[] shape)
{
...
boolean keyCheck(char keyASCII, int keyCodeID)
{
    if (key == CODED)
...
boolean isBackToBack(Trick trick)
{
...
public class Block
{
    ...
    public Block(color blockColor, Vector2 cell)
    {
        this.cell = cell;
...
public class Tetromino
{
    ...
    Vector2 getShapeSize(Block[]blocks)
    {
        int maxX = 0, maxY = 0;
        for (int i = 0; i < blocks.length; i++)
        {
